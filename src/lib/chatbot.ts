...
export const companyInfo = {
  ...
  description: "We are a remote software development company working with clients globally, specializing in building innovative tech solutions that make a difference.",
  ...
  whyChooseUs: [
    "Remote team serving clients globally with competitive rates",
...
export function generateBotResponse(userMessage: string): string {
  ...
  if (message.match(/(ano.*company|what.*company.*name|ano.*pangalan.*company|name.*company)/i)) {
    return `🏢 Our company is Nexxus Lab!\n\n"${companyInfo.tagline}"\n\nWe're a remote tech company specializing in software development, AI solutions, and digital transformation - serving clients globally.`;
  }
  ...
  if (message.match(/(service|offer|provide|do you do|what do you|ano ginagawa|serbisyo|ano.*services)/i)) {
    const serviceList = companyInfo.services.map(s => `• ${s.name}`).join("\n");
  ...
  if (message.match(/(contact|reach|email|phone|number|call|message|how to contact|paano kayo macontact)/i)) {
    return `📞 Contact Us\n\n📱 ${companyInfo.contact.phone}\n✉️ ${companyInfo.contact.email}\n🌐 ${companyInfo.contact.website}\n\nWe're a remote company serving clients globally. Feel free to reach out anytime!`;
  }
  
  // Location
  if (message.match(/(where|location|address|office|based|saan|lugar|nasaan)/i)) {
    return `📍 Our Location\n\nWe're based in Makati, Philippines.\n\nWe work with clients globally! Interested in working with us? Reach out at ${companyInfo.contact.email}`;
  }
